describe("Neosavvy.Core.Builders.RequestUrlBuilder", function () {
    ...
    describe("addParam", function () {
        it("Should throw an error if both the key and value are undefined", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").addParam();
            }).toThrow();
        });

        it("Should throw an error if the key is defined but not an object", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").addParam("myKey");
            }).toThrow();
        });

        it("Should throw an error if the user has attempted to overwrite and existing key", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").addParam("myKey", 50).addParam("myKey", 60);
            }).toThrow();
        });

        it("Should throw an error if passed objects with overlapping keys", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").addParam({name: "Steve"}).addParam({name: "Tom"});
            }).toThrow();
        });

        it("Should return the builder", function () {
            var builder = new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com");
    ...
    describe("paramReplace", function () {
        it("Should throw an error for a double replace", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com/:myParam").paramReplace(":myParam", 5).paramReplace(":myParam", 6);
            }).toThrow();
        });
        ...
        it("Should throw an error if the key is not an object but provided", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com/:name").paramReplace(":name");
            }).toThrow();
        });

        it("Should throw an error if both arguments are undefined", function () {
            expect(function () {
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com/:myParam").paramReplace();
            }).toThrow();
        });
    });
    ...
    describe("build", function () {
        ...
        it("Should return the baseUrl if no params are provided", function () {
            expect(new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").build()).toEqual("http://www.neosavvy.com");
        });

        it("Should return the baseUrl with params if there are some", function () {
            expect(
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com").
                    addParam("name", 40).
        ...
        it("Should be able to return a url with replaced params", function () {
            expect(
                new Neosavvy.Core.Builders.RequestUrlBuilder("http://www.neosavvy.com/:house/story").
