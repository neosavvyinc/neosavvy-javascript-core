describe("_ns.DomUtils", function () {
    describe("getElementsByAttribute", function () {
        ...
        it("Should return one item match the first-name attribute", function () {
            expect(_ns.DomUtils.getElementsByAttribute('div', 'first-name', "clark").length).toEqual(1);
        });

        it("Should return two items matching the first-name attribute", function () {
            expect(_ns.DomUtils.getElementsByAttribute('span', 'first-name', "clark").length).toEqual(2);
        });

        it("Should return one item matching the age attribute", function () {
            expect(_ns.DomUtils.getElementsByAttribute('div', 'age', "49").length).toEqual(1);
        });

        it("Should return two items matching the age attribute", function () {
            expect(_ns.DomUtils.getElementsByAttribute('a', 'age', "36").length).toEqual(2);
        });

        it("Should be able to pick up all the ones where an attribute is undefined", function () {
            expect(_ns.DomUtils.getElementsByAttribute('div', 'first-name', undefined).length).toEqual(1);
        });
